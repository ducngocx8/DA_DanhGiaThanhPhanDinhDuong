...
import FormSearchThucPhamEdit from "./FormSearchThucPhamEdit";

export default function FormEditMonAn({
  closeFormAdd,
  loadData,
  itemChoose,
  params,
  ...
  useEffect(() => {
    async function getAllNhomMonAn() {
      const response = await axios.get(`${NhomMonAnAdminURL}`, {
    ...
    async function getAllChiTietMonOfMonAn() {
      const response = await axios.get(
        `${ChiTietMonAdminURL + "/" + itemChoose.id_monan}`,
        {
          withCredentials: true,
        }
      );
  ...
  const handleUpdateChiTietMon = async () => {
    ...
    const response = await axios.put(
      `${ChiTietMonAdminURL + "/" + infoChiTietMonEdit.id_chitietmon}`,
      newChiTietMon,
      {
        withCredentials: true,
      }
  ...
  const editProduct = async (newProduct) => {
    const response = await axios.put(
      `${MonAnAdminURL + "/" + itemChoose.id_monan}${params}`,
      newProduct,
      {
        withCredentials: true,
      }
    ...
    >
      {showConfirmDeleteChiTietMon && (
        <div className="background_black_child" style={{ display: "block" }}>
          <FormConfirm
            status={"admin_edit_delete_chitietmon"}
            content={
              "Xác nhận xóa " +
