...
export default function FormAddBaiViet({
  closeFormAdd,
  loadData,
  username,
  params,
  ...
  const handleChangeValue = (e) => {
    ...
    if (name === "id_chuyenmuc") {
      ...
      setEditSlug(false);
      const slug_value = getSlug(value);
      setBaiVietInput({ ...baiVietInput, slug: slug_value, [name]: value });
      return;
    }
  ...
  const AddBaiViet = async (newItem) => {
    const response = await axios.post(`${BaiVietAdminURL}${params}`, newItem, {
      withCredentials: true,
    });
