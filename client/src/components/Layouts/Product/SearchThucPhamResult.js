...
import { BACKEND_HOME, NhomThucPhamURL } from "../../../api";
import { useNavigate, useSearchParams } from "react-router-dom";
import axios from "axios";

export default function SearchThucPhamResult({
  productList,
  imageUrl,
  isPagination,
  keyword,
  ...
  const navigatePage = (sort_type, category_id) => {
    ...
    return navigate(
      `/dinh-duong/search?keyword=${keyword}&sort_type=${sort_type}&energy_from=${calo_ref.current[0]}&energy_to=${calo_ref.current[1]}&protein_from=${protein_ref.current[0]}&protein_to=${protein_ref.current[1]}&fat_from=${fat_ref.current[0]}&fat_to=${fat_ref.current[1]}&cabs_from=${cabs_ref.current[0]}&cabs_to=${cabs_ref.current[1]}${category_query}`,
      { replace: true }
    );
  };
