...
export default function MonAnDungNhieu({ menuName, imageUrl, setLoading }) {
  let [monAnList, setMonAnList] = useState([]);

  useEffect(() => {
    async function getMonAnDungNhieu() {
      const response = await axios.get(`${MonAnURL + "/get/dung-nhieu"}`, {
  ...
  console.log("DUNG NHIEU", monAnList);
  return (
                  ...
                  product_detail={{
                    ...
                    product_image:
                      product?.image_url.length > 0
                        ...
                        : noImage,
                    energy_index: Number(product.TOTAL) * 1,
                    don_vi: "phần ăn",
