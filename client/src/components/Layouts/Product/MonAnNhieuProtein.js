...
import { BACKEND_HOME, DuongChatMonAnOffset } from "../../../api";
...
export default function MonAnNhieuProtein({ menuName, imageUrl, setLoading }) {
  let [monAnList, setMonAnList] = useState([]);

  useEffect(() => {
    async function getMonAnNhieuProtein() {
      const response = await axios.get(
        `${DuongChatMonAnOffset + "?code=PROCNT&offset=0&limit=20"}`,
        {
                  ...
                  product_detail={{
                    ...
                    product_image:
                      product?.image_url.length > 0
                        ...
                        : noImage,
                    energy_index: Number(product.PROCNT).toFixed(0),
                    don_vi: "gram",
