...
export default function DuongChatMonAn({ navigation }) {
  let limit = 30;
    ...
    if (duongChatSelected !== "") {
      const getAllThucPhamDuongChat = async () => {
        const response = await axios.get(
          `${MonAnURL}/tra-cuu/duong-chat/offset?code=${duongChatSelected}&sort=${sortType}&offset=${startOffset}&limit=${limit}`,
          {
  ...
  const handleLoadMore = async () => {
    if (!endList) {
      ...
      const response = await axios.get(
        `${MonAnURL}/tra-cuu/duong-chat/offset?code=${duongChatSelected}&sort=${sortType}&offset=${newOffset}&limit=${limit}`,
        {
          withCredentials: true,
        }
      );
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text style={styles.column50_thucpham}>
                        {item.ten_mon
                          ? item.ten_mon.length > 15
                            ? item.ten_mon.slice(0, 15) + "..."
                            : item.ten_mon
                          : "Chưa đặt tên"}
                      </Text>
